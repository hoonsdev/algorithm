...
function solution(input) {
  // 윗면 꼭짓점 3면 노출 -> 4개 : min(a~f)
  // 아랫면 꼭짓점 2면 노출 -> 모서리랑 같은 기능
  // 윗면, 옆면 모서리 2면 노출 -> 전부 모서리로 생각하면 -> 4(N-2) + 4(N-1) = 8N - 12 개 : (1) (2) (3) 각각 최솟값 구해서 그 중 최소 2개 더하기
  // 나머지 제일 작은 수로 조정 가능 -> 4(N-1)(N-2) + (N-2)(N-2) 개 : (1) (2) (3) 최솟값 3개 더하기
  let answer;
  const N = +input[0];
